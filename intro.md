...
const expect = jQuery.expect;

3. Why would we bring in jQuery as a `dependency` rather than a `devDependency`?
because we would want the jQuery to work for the whole project, not just what the dev is doing

4. Looking at the documentation, what does `.addClass()` do?
this will add a class to the selected element
